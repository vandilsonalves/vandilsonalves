...
async def get_trending(limite: int = Query(10, ge=1, le=20)):
    """Retorna os posts mais populares das últimas 24 horas"""
    ...
    for post in posts:
        ...
        comentarios = await db.feed_comentarios.count_documents({"post_id": post["id"]})
        post["engajamento"] = curtidas * 2 + comentarios * 3  # Comentários valem mais

...
# ==================== ATLETA - TROCA DE EQUIPE ====================

class TrocaEquipeRequest(BaseModel):
    nova_equipe: str


@api_router.get("/atletas/status-troca-equipe")
async def get_status_troca_equipe(current_user: dict = Depends(get_current_user)):
    ...
    # Se for admin, não pode trocar (não é atleta)
    if current_user.get("role") == "admin":
        return {
            "pode_trocar": False,
            "motivo": "admin",
...
@api_router.post("/atletas/trocar-equipe")
async def trocar_equipe_atleta(
    ...
    if current_user.get("role") == "dono_assessoria":
        raise HTTPException(
            status_code=403, 
            detail="Donos de assessoria não podem trocar de equipe. Entre em contato com o administrador."
        )
    
    # Verificar última troca
    ultima_troca = current_user.get("ultima_troca_equipe")
    if ultima_troca:
        try:
            data_ultima_troca = datetime.fromisoformat(ultima_troca)
            ...
            pass  # Data inválida, permitir troca
    ...
    # Verificar se a equipe existe (se não for INDIVIDUAL)
    if nova_equipe.upper() != "INDIVIDUAL":
        # Verificar se existe algum atleta ou assessoria com esse nome
        ...
        if not equipe_existe and not assessoria_existe:
            raise HTTPException(
                status_code=404, 
                detail="Equipe não encontrada. Verifique o nome ou escolha INDIVIDUAL."
            )
    
    # Atualizar equipe e data da última troca
    ...
    return {
        ...
        "proxima_troca_disponivel": (datetime.now() + td(days=15)).strftime("%d/%m/%Y")
    }
